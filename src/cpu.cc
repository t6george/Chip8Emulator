...
void Chip8Cpu::run(Peripherals &peripherals)
{
  ...
  case 0x3000:
  { //0x3XNN: skips next opcode if VX == NN
  ...
  case 0x8000:
  {
    ...
    case 0x0004:
    { //0x8XY4: add VX and VY into VX and save carry into VF
      int X = (opcode & 0x0F00) >> 8;
      int Y = (opcode & 0x00F0) >> 4;
      if (V[X] > 0xFF - V[Y])
      { //check for overflow
    ...
    case 0x0005:
    { //0x8XY5: perform VX-VY and set VF to complimented carry
      int X = (opcode & 0x0F00) >> 8;
      int Y = (opcode & 0x00F0) >> 4;
      if (V[Y] > V[X])
      {
    ...
    case 0x0006:
    { //0x8XY6: shift VX 1 bit to the right (do not change VY)
    ...
    case 0x0007:
    { //0x8XY7: perform VY-VX and set VF to complimented carry
      int X = (opcode & 0x0F00) >> 8;
      int Y = (opcode & 0x00F0) >> 4;
      if (this->V[X] > this->V[Y])
      {
    ...
    case 0x000E:
    { //0x8XY6: shift VX 1 bit to the left (do not change VY)
  ...
  case 0xA000:
  { //0xANNN: set address register to NNN
  ...
  case 0xC000:
  { //0xCXNN: set V[X] to a random number & NN
    int X = (opcode & 0x0F00) >> 8;
    int NN = opcode & 0x00FF;
  ...
  case 0xD000:
  { //DXYN: draw sprite of width 8px, height Npx, at coords (VX,VY)
  ...
  case 0xE000:
  {
    ...
    case 0x009E:
    { //0xEX9E: skip next instruction if key V[X] is pressed
    ...
    case 0x00A1:
    { //0xEXA1: skip next instruction if key V[X] is not pressed
      int key = this->V[(opcode & 0x0F00) >> 8];
  ...
  case 0xF000:
  {
    ...
    case 0x0029:
    { //0xFX29: sets I to point to character sprite in V[X]
    ...
    case 0x000A:
    { //0xFX0A: await a keypress and store in VX
      int X = (opcode & 0x0F00) >> 8;
      bool pressed = false;
      for (int i = 0; i < 16; i++)
      {
    ...
    case 0x001E:
    { //0xFX1E: set I to VX + I and VF is set to the overflow
      int X = (opcode & 0x0F00) >> 8;
    ...
    case 0x0033:
    { //0xFX33: store a binary-coded decimal V[X]'s digits in I, I+1, I+2
    ...
    case 0x0065:
    { //0xFX65: loads data to register V[0] to V[X] starting from mem address I
