...
Peripherals::Peripherals(int mode){
	
  this->gfx = new short *[32];									//create an initially cleared screen
  for(int i = 0; i < 32; i++){
	...
	if(mode == 0){																//Emulator is being run with Allegro
		...
		this -> canvas = NULL;
	}else{																				//Emulator is being run on 64x32 LED Matrix
